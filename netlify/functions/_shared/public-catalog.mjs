import { artistIdFor, validArtistId, artistAvatarUrl } from './artist-identity.mjs';
...
export function publicRelease(draft) {
    ...
    const artistId=artistIdFor(draft.ownerId, release.artist);
...
export function createCatalogHandler(getStore) {
    return async(request,context)=>{
        try{
            ...
            if(url.pathname.endsWith('/artist-avatar')) {
                const id=url.searchParams.get('id');
                if(!validArtistId(id))return json({error:'not_found'},404);
                const profile=await store.get(`artist-profiles/${id}.json`,{type:'json'});
                const match=/^data:(image\/(?:png|jpeg|webp));base64,(.+)$/.exec(profile?.avatar || '');
            ...
            for(const item of profiles.blobs) {
                const p=await store.get(item.key,{type:'json'});
                if(!p)continue;
                artists.set(p.id,{id:p.id,name:p.name,slug:p.id,bio:p.bio,
                    avatar:artistAvatarUrl(p)});
            }
